...
const LegalRequestNewPage: React.FC = () => {
  ...
  // The requester's department comes from the company directory (the auth
  // profile only carries the core identity fields).
  const { personFor } = useLegalPeople();
  const department = personFor(user?.id)?.department ?? '';
    ...
    <LegalShell title="New legal request" description="Three short steps. The legal team is notified as soon as you submit." backTo={{ label: 'Back to requests', path: LEGAL_PATHS.requests }}>
      <Card>
        <CardContent className="p-6">
          <IntakeWizard
            defaults={{ requester_department: department }}
