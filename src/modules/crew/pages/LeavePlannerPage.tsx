...
import {
  ...
  Undo2, Loader2, Info, CalendarDays, Download, Calculator, AlertTriangle,
} from 'lucide-react';
...
type SortKey = 'name' | 'rank' | 'department' | 'remaining' | 'taken' | 'next';
...
export default function LeavePlannerPage() {
  ...
  const { vessels, selectedVessel, isAllVessels, canAccessAllVessels } = useVessel();

  const isHoD = ['master', 'chief_engineer', 'chief_officer'].includes(profile?.role || '');
  const departmentScope = isHoD && departmentFilter === 'All' ? null : (departmentFilter === 'All' ? null : departmentFilter);

  const dragRef = useRef<{ active: boolean; crewId: string; startDay: number; currentDay: number; code: string } | null>(null);
  ...
  const filteredCrew = useMemo(() => {
    ...
    if (search) {
      ...
      data = data.filter(c =>
        ...
        (c.vesselName || '').toLowerCase().includes(s),
      );
    }
    return data;
  ...
  const groupedCrew = useMemo(() => {
    ...
    const ordered = [...filteredCrew].sort((a, b) => {
  ...
  const sortedCrew = useMemo(() => {
    const data = [...filteredCrew];
    const dir = sortDir === 'asc' ? 1 : -1;
    data.sort((a, b) => {
      switch (sortKey) {
        case 'name': return a.lastName.localeCompare(b.lastName) * dir;
        case 'rank': return (a.position || '').localeCompare(b.position || '') * dir;
        case 'department': return (a.department || '').localeCompare(b.department || '') * dir;
        case 'remaining': return (a.remaining - b.remaining) * dir;
        case 'taken': return (a.taken - b.taken) * dir;
        case 'next': return ((a.nextLeaveStart || '￿').localeCompare(b.nextLeaveStart || '￿')) * dir;
        default: return 0;
      }
    });
    return data;
  }, [filteredCrew, sortKey, sortDir]);
  ...
  const exportCsv = useCallback(() => {
    const header = [
      'Last Name', 'First Name', 'Vessel', 'Department', 'Rank',
      'Rotation', 'Employment Start', 'Contract Start', 'Contract End',
      'Annual Entitlement', 'Carryover', 'Accrued', 'Taken', 'Booked',
      'Pending', 'Adjustments', 'Remaining', 'Next Leave Start',
      'Next Leave End', 'Notes',
    ];
    ...
    link.href = url;
    const scope = effectiveVesselId
      ? (selectedVessel?.name || 'vessel')
      : 'fleet';
    link.download = `leave-planner-${scope}-${year}-${month}.csv`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  }, [sortedCrew, year, month, effectiveVesselId, selectedVessel]);
    ...
    <DashboardLayout>
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2 flex-wrap bg-card border rounded-lg p-3">
          <Tabs value={viewMode} onValueChange={(v) => setViewMode(v as ViewMode)}>
            <TabsList className="h-8">
              ...
              <TabsTrigger value="table" className="text-xs h-7">Planner Table</TabsTrigger>
            </TabsList>
          </Tabs>
        ...
        {filteredCrew.length === 0 && (
          <div className="border rounded-lg bg-card p-8 text-center text-sm text-muted-foreground flex flex-col items-center gap-2">
            <AlertTriangle className="w-6 h-6 text-amber-500" />
            <div>
              No crew match the current filters.
              {effectiveVesselId
                ? ' Crew must be assigned to this vessel via Crew Assignments to appear.'
                : ' Select a vessel from the top bar, or switch to Fleet View if you have access.'}
            </div>
          </div>
        )}

        {viewMode === 'calendar' && filteredCrew.length > 0 && (
          <div className="border rounded-lg overflow-hidden bg-card">
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-xs">
                <thead>
                  <tr className="bg-muted/50">
                    <th className="sticky left-0 z-20 bg-muted/90 backdrop-blur-sm text-left px-2 py-1.5 border-r font-medium min-w-[180px]">Crew Member</th>
                    <th className="sticky left-[180px] z-20 bg-muted/90 backdrop-blur-sm text-left px-2 py-1.5 border-r font-medium min-w-[100px]">Position</th>
                    <th className="sticky left-[280px] z-20 bg-muted/90 backdrop-blur-sm text-right px-2 py-1.5 border-r font-medium min-w-[55px]">Balance</th>
                    ...
                    {SUMMARY_CODES.map(code => (
                      <th key={code} className="text-center px-1 py-1 font-medium border-l min-w-[28px]" style={{ color: STATUS_CODE_MAP[code]?.color }}>
                        {code}
                      </th>
                    ))}
                  </tr>
                  ...
                  {Object.entries(groupedCrew).map(([dept, members]) => (
                    <React.Fragment key={dept}>
                      <tr className="bg-muted/30">
                        <td colSpan={3 + daysInMonth + SUMMARY_CODES.length} className="sticky left-0 z-10 px-2 py-1.5 font-semibold text-xs uppercase tracking-wider text-muted-foreground">
                      ...
                      {members.map(crew => (
                        <tr key={crew.userId} className="hover:bg-muted/20 border-t border-border/50">
                          <td className="sticky left-0 z-10 bg-card px-2 py-1 border-r whitespace-nowrap font-medium">
                            ...
                            {crew.vesselName && (
                              <span className="ml-1 text-[10px] text-muted-foreground">· {crew.vesselName}</span>
                            )}
                          </td>
                          ...
                          {SUMMARY_CODES.map(code => (
                            <td key={code} className="text-center border-l text-[10px] font-medium px-0.5" style={{ color: STATUS_CODE_MAP[code]?.color }}>
                              {crew.counts[code] || 0}
                            </td>
                          ))}
                        </tr>
        ...
        {viewMode === 'table' && filteredCrew.length > 0 && (
          <div className="border rounded-lg overflow-hidden bg-card">
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead className="bg-muted/50">
                  <tr>
                    <th className="text-left px-2 py-2 border-b">{sortHeader('name', 'Crew Member')}</th>
                    <th className="text-left px-2 py-2 border-b">Vessel</th>
                    <th className="text-left px-2 py-2 border-b">{sortHeader('department', 'Department')}</th>
                    <th className="text-left px-2 py-2 border-b">{sortHeader('rank', 'Rank')}</th>
                    <th className="text-left px-2 py-2 border-b">Rotation</th>
                    <th className="text-left px-2 py-2 border-b">Employment Start</th>
                    <th className="text-right px-2 py-2 border-b">Entitlement</th>
                    <th className="text-right px-2 py-2 border-b">Carry</th>
                    <th className="text-right px-2 py-2 border-b">Accrued</th>
                    <th className="text-right px-2 py-2 border-b">{sortHeader('taken', 'Taken')}</th>
                    <th className="text-right px-2 py-2 border-b">Booked</th>
                    <th className="text-right px-2 py-2 border-b">{sortHeader('remaining', 'Remaining')}</th>
                    <th className="text-left px-2 py-2 border-b">{sortHeader('next', 'Next Leave')}</th>
                    <th className="text-left px-2 py-2 border-b">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {sortedCrew.map((c) => (
                    <tr key={c.userId} className="border-b hover:bg-muted/20">
                      <td className="px-2 py-1.5 font-medium whitespace-nowrap">{c.lastName}, {c.firstName}</td>
                      <td className="px-2 py-1.5 whitespace-nowrap">{c.vesselName || '—'}</td>
                      <td className="px-2 py-1.5">{c.department}</td>
                      <td className="px-2 py-1.5">{c.position}</td>
                      <td className="px-2 py-1.5">{c.rotation || '—'}</td>
                      <td className="px-2 py-1.5">{c.employmentStart || c.contractStart || '—'}</td>
                      <td className="px-2 py-1.5 text-right">{c.annualEntitlement}</td>
                      <td className="px-2 py-1.5 text-right">{c.carryover}</td>
                      <td className="px-2 py-1.5 text-right">{c.accrued}</td>
                      <td className="px-2 py-1.5 text-right">{c.taken}</td>
                      <td className="px-2 py-1.5 text-right">{c.booked}</td>
                      <td className={cn(
                        'px-2 py-1.5 text-right font-bold',
                        c.remaining < 0 && 'text-red-600',
                        c.remaining > 40 && 'text-green-600',
                      )}>
                        {c.remaining}
                      </td>
                      <td className="px-2 py-1.5 whitespace-nowrap">
                        {c.nextLeaveStart
                          ? `${c.nextLeaveStart}${c.nextLeaveEnd && c.nextLeaveEnd !== c.nextLeaveStart ? ` → ${c.nextLeaveEnd}` : ''}`
                          : '—'}
                      </td>
                      <td className="px-2 py-1.5">
                        {c.accrualNotes.length > 0 ? (
                          <span className="text-amber-600 text-[10px]" title={c.accrualNotes.join('\n')}>⚠ {c.accrualNotes.length}</span>
                        ) : c.isImported ? (
                          <span className="text-blue-600 text-[10px]">imported</span>
                        ) : (
