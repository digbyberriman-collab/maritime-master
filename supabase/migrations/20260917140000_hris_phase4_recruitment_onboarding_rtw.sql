...
-- =================================================================
-- 1. vacancies
-- 2. candidates (non-auth person records) + candidate_applications
--    + application_events (pipeline history) + interviews
-- 3. onboarding_templates / onboarding_records / onboarding_items
--    (HR-side induction; complements ISM familiarisation)
-- 4. right-to-work: crew_work_authorisations, hr_generate_alerts()
--    (turns hr_expiry_items / hr_performance_due_items into `alerts`
--    rows, idempotent), pg_cron schedule when available
-- =================================================================
...
-- ---------------------------------------------------------------
-- 2. candidates + applications + events + interviews
-- ---------------------------------------------------------------
...
-- Hire: creates the crew profile (imported-style, no login yet), links it to
-- the candidate, marks the application hired and the vacancy filled when
-- headcount is reached, and opens an onboarding record if a template exists.
...
-- ---------------------------------------------------------------
-- 3. Onboarding (HR induction) templates / records / items
-- ---------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.onboarding_templates (
  ...
  -- [{ "section": "Before joining", "items": [{ "title": "Signed SEA received", "owner": "hr", "due_offset_days": -7, "required": true }] }]
...
-- Start onboarding for a joiner: explode the template into items.
...
-- Add work authorisations to the expiry feed.
...
-- hr_generate_alerts(): turns due/expiry items into `alerts` rows so the
-- existing Alerts module surfaces them. Idempotent per (entity, alert_type):
-- an open alert is updated, a resolved one is not re-opened unless the date
-- moved. Thresholds: 90 days (YELLOW), 30 days (ORANGE), overdue (RED).
