-- Allow privileged crew managers to write staged crew rows
CREATE POLICY "crew_import_write_privileged" ON public.crew_import
  FOR INSERT TO authenticated
  WITH CHECK (public.has_any_role(auth.uid(), ARRAY['superadmin','dpa','fleet_master','captain','purser']::app_role[]));

CREATE POLICY "crew_import_update_privileged" ON public.crew_import
  FOR UPDATE TO authenticated
  USING (public.has_any_role(auth.uid(), ARRAY['superadmin','dpa','fleet_master','captain','purser']::app_role[]))
  WITH CHECK (public.has_any_role(auth.uid(), ARRAY['superadmin','dpa','fleet_master','captain','purser']::app_role[]));

GRANT SELECT, INSERT, UPDATE ON public.crew_import TO authenticated;
GRANT ALL ON public.crew_import TO service_role;

-- Promote staged crew rows into the operational crew roster (profiles)
...
BEGIN
  ...
  LOOP
    ...
    -- Match: import reference, then email, then first+last name
    v_profile_id := NULL;
