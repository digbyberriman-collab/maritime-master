-- Fix vessel dashboard function to use correct enum values
...
BEGIN
  ...
  ELSE
    ...
    SELECT 
      ...
      COALESCE((SELECT COUNT(*) FROM alerts a WHERE a.vessel_id = v.id AND a.status = 'OPEN'::alert_status), 0)::BIGINT AS open_alerts_count,
      COALESCE((SELECT COUNT(*) FROM alerts a WHERE a.vessel_id = v.id AND a.status = 'OPEN'::alert_status AND a.severity_color = 'red'), 0)::BIGINT AS red_alerts_count,
