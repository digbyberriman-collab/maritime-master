...
-- =================================================================
-- 1. performance_competencies (company rating framework, seeded)
-- 2. performance_review_cycles
-- 3. performance_reviews (annual evaluation / annual review /
--    end-of-rotation catch-up / probation / ad hoc)
-- 4. crew_objectives + crew_objective_updates (Objectives & PDPs)
-- 5. disciplinary_records (+ incident_involved_persons normalisation)
-- 6. notification types + hr_performance_due_items view
-- 7. RPCs: performance_review_submit_self_assessment,
--    performance_review_acknowledge, disciplinary_record_acknowledge
-- =================================================================

-- ---------------------------------------------------------------
-- 1. performance_competencies
-- ---------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.performance_reviews (
  ...
  -- [{ "competency_id": uuid, "name": "Safety awareness", "rating": 4, "comment": "" }]
  ...
  welfare_notes text,
  -- [{ "action": "...", "owner": "reviewer|employee|hr", "due_date": "2026-12-01", "done": false }]
...
CREATE POLICY "performance_reviews_select" ON public.performance_reviews
  FOR SELECT USING (
    ...
    AND (
      ...
      -- the subject sees their review once it reaches them (never drafts)
...
-- Subject-side RPCs: the subject is not allowed a blanket UPDATE policy, so
-- self-assessment and acknowledgement go through SECURITY DEFINER functions
-- that only touch their own columns.
...
-- ---------------------------------------------------------------
-- 4. crew_objectives + crew_objective_updates
-- ---------------------------------------------------------------
...
-- Progress notes roll up to the objective.
...
-- ---------------------------------------------------------------
-- 5. disciplinary_records (+ incident_involved_persons)
-- ---------------------------------------------------------------
...
CREATE POLICY "disciplinary_records_select" ON public.disciplinary_records
  FOR SELECT USING (
    ...
    AND (
      ...
      -- the subject sees the outcome, never the investigation file (UI hides those columns; see disciplinary_records_self view)
...
-- Subject-safe projection (no investigation notes / witness statements).
...
-- Warnings lapse on their expiry date (nightly with pg_cron if present).
...
-- ---------------------------------------------------------------
-- 6. Notification types + due-items view
-- ---------------------------------------------------------------
