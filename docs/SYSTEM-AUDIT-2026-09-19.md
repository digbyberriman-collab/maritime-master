...
| H5 | ISM form signature workflow unenforced; client-side sequence numbers for audits, drills, findings; fake success toasts for exports; mock fallback on query error. | `useFormSubmissions.ts:454-548`; `useAudits.ts:151`; `useDrills.ts:194`; `FormExports.tsx:86`; `Drawings.tsx:93` | Signed ISM records are not trustworthy as evidence; numbers collide across users; outages are masked. **Addressed 19 Sep 2026** for the signature workflow, all four numbering paths and the export toast, in `20260919210000_ism_form_workflow.sql` and `20260919220000_reference_numbers.sql`; the `Drawings.tsx` mock fallback remains open. |
| H6 | ISM template status vocabulary split (`DRAFT/PUBLISHED/ARCHIVED` vs `draft/active`). | `DraftTemplates.tsx:43,71` vs `useFormTemplates.ts:163,184`; live values `PUBLISHED=6, ARCHIVED=1`, no CHECK | Drafts page never lists drafts; publishing from it writes an out-of-vocabulary status. **Addressed 19 Sep 2026**: CHECK constraints on all three tables, a fourth dead vocabulary (`forms/types.ts`) deleted, and the seven surfaces reading `draft`/`active`/`completed`/`pending_signatures` repointed. |
...
### Work completed after this report was issued

This section records remediation carried out on the same day, so the findings
above can be read against the current state rather than the state at the time
of the audit.

| Action | Findings | What changed | Verification |
|---|---|---|---|
| 2.5 (a) ISM signature workflow enforced server-side | H5, H6 | `20260919210000_ism_form_workflow.sql`: status CHECK constraints on `form_templates`, `form_submissions` and `form_signatures`; a signer-role vocabulary (`form_normalize_signer_role`, `form_user_satisfies_signer_role`) replacing a comparison against `profiles.rank` that could never match; a BEFORE trigger (`form_submissions_before_write`) that owns identity, company, numbering, the sha256 content hash, the submit stamp, the lock and every status transition; `form_sign_submission` / `form_reject_submission` / `form_pending_signatures` as the only write paths for signatures, with signatures made append-only (`REVOKE INSERT, UPDATE, DELETE`); signing cycles so a rejected round stays on file; a `WITH CHECK` on the submission UPDATE policy, which previously allowed a cross-tenant move; a DELETE policy, absent before, so deleting a draft now works | 47 assertions against a local PostgreSQL 16 instance carrying a scaffold of the live schema, plus an 11-step smoke test against the live database in a rolled-back transaction using real crew, master and chief-officer accounts. Both confirm a crew member can no longer set `status = 'SIGNED'`, edit a locked form, unlock it, move it between companies, or write a signature row. |
| 2.5 (b) Reference numbers moved into the database | H5 | `20260919220000_reference_numbers.sql`: `reference_counters` plus `next_reference_value`, and triggers numbering `audits`, `drills` and `audit_findings`. All four numbering paths (including form submissions) now advance atomically. Uniqueness was rescoped to match: `audits` and `drills` had a global `UNIQUE` that per-tenant numbering would collide on, and `audit_findings.finding_number` had no constraint at all | 150 concurrent calls produced 150 distinct numbers; the same test against the previous `max()+1` approach produced 10 distinct values from 60 calls. The collision between per-tenant numbering and the global `UNIQUE` was caught locally before the migration reached the live database. |
| 2.6 (partial) Fake success path removed | H5 | Bulk form export reported "Export completed!" after a two-second `setTimeout` without producing a file. It now says bulk export is unavailable | — |
| Broken queries fixed alongside | H6, H9 | `FormsArchive` selected four columns that do not exist (`submitted_by_name`, `vessel_name`, `data`, `form_templates.name`), so the archive was always empty; `ChecklistsPage` compared `form_type` to a lower-case literal against an upper-case column | Full suite: 799 tests, typecheck and lint clean, production build succeeds |
| Duplicate signature path removed | consolidation | `useSMSForms.ts` was a second, weaker implementation of the same workflow against the empty `sms_*` tables, exported from the module barrel but imported by no page. Deleted rather than hardened twice. The `sms_*` tables themselves were left in place | Wiring test asserts it does not return |

The `sms_*` tables remain on the live database, empty and now unreferenced by
any code. Dropping them is a separate decision.
